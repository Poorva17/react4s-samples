...
case class CodeLoaderComponent(path : P[String], symbol : P[Option[String]], highlight : P[Boolean]) extends Component[NoEmit] {
    ...
    val prefix = "https://raw.githubusercontent.com/Ahnfelt/react4s-samples/master/src/main/scala/com/github/ahnfelt/react4s/samples/"
    ...
    def filterCode(get : Get, code : String) = {
        val noImports = if(get(showImports)) code else {
            code.lines.dropWhile(l =>
                l.trim.isEmpty ||
                l.startsWith("import") ||
                l.startsWith("package")
            ).mkString("\n")
        }
        get(symbol).filter(_ => !get(showImports)).map(s =>
            noImports.lines.
                dropWhile(l => !l.contains(s)).
    ...
    override def render(get : Get) : Element = {
        E.div(
            ...
            else Component(CodeComponent, get(loader.result).map(filterCode(get, _)).getOrElse(""), get(highlight))
        )
    }
}
